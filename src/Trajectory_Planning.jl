...
dt_horizon = 0.2           # Time-step length per horizon
Nt_horizon = Int(hor/dt)+1  # Number of timesteps per horizon
...
N=2                 #Number of UAVs.
dt_sim = 1          #Timestep of whole simulation.
Nt_sim = tf/dt_sim  #Number of timesteps in simulation.


# #Give hard-coded initial and final(target) conditions. FOR TESTING PURPOSES.
...
for q in eachindex(single_input_pb) #for each epoch
    println("===Epoch No. $q starts===")
    if q ==1
        global x_start = TDM_TRAJECTORY_opt.GreensPb_to_ALTRO(single_input_pb[q])
    end
    global x_final = TDM_TRAJECTORY_opt.GreensPb_to_ALTRO(single_output_pb[q]) #contains a list of the 13 final states for each drone in the current epoch.
    ...
    for i in 1:N
        ...
        push!(junc_state, RBState(X[i][end,:]))
    end
